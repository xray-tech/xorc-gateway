...
impl AppRegistry {
    ...
    /// `[postgres]` section, the system loads the data from Cassandra sdk
    /// database, from the applications, ios_applications, android_applications
    /// and web_applications tables.
    ///
    ...
    /// * Does the `XORC-Api-Token` header exist,
    /// * Is the given `XORC-Api-Token` header same as in database or configuration,
    /// * If `allow_empty_signature` is set to `false`, is the `XORC-Signature`
    ...
    fn update_apps(&self) -> Result<(), io::Error> {
        if let Some(ref pool) = self.pool {
            let query = QueryBuilder::new(
                format!(
                    "SELECT id, sdk_token, ios_api_secret, android_api_secret, web_api_secret FROM {}.applications",
            ...
            if let Some(rows) = rows {
                let apps = rows.iter().fold(HashMap::new(), |mut acc, row| {
                    ...
                    let ios_secret: Option<String>     = row.by_name("ios_api_secret").unwrap();
                    let web_secret: Option<String>     = row.by_name("web_api_secret").unwrap();
                    let android_secret: Option<String> = row.by_name("android_api_secret").unwrap();
